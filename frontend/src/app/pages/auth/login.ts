            ...
            <div class="border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 rounded py-16 px-6 md:px-16 z-10">
                ...
                <div class="flex flex-col">
                    <p-iconfield class="w-full mb-6">
                        ...
                        <input id="email" type="text" pInputText class="w-full md:w-100" [(ngModel)]="nombre" placeholder="Usuario" />
                    </p-iconfield>

                    <p-iconfield class="w-full mb-6">
                        <p-inputicon class="pi pi-lock" />
                        <input [(ngModel)]="password" id="password" type="password" pInputText class="w-full md:w-100" placeholder="Password" />
                    </p-iconfield>

                    <button pButton pRipple label="Log In" class="w-full" (click)="loguearse()"></button>
                </div>
            </div>
        </div>
...
export class Login {
    ...
    async iniciarSession(){
        ...
        await this.auth.login(this.nombre(), this.password());
    }

    loguearse(){
        // Validación básica
        if(this.nombre().trim().length === 0 || this.password().trim().length === 0){
            // Usamos alerta simple si no hubo datos (puedes reemplazar por SweetAlert si prefieres)
